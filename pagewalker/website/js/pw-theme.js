(function () {
  ...
  function apply() {
    ...
    if (meta) meta.setAttribute("content", "#16140f");
    applyLogoTheme();
  }
    ...
    logoPromise = new Promise(function (resolvePromise) {
      ...
      img.onload = function () {
        try {
          ...
          logoSrc = cropCanvasToOpaque(canvas).toDataURL("image/png");
          resolvePromise(true);
        } catch (_) {
  ...
  function setupLogos() {
    ...
    buildLogoVariant().then(applyLogoTheme);
  }

  apply();

  window.pwTheme = { apply: apply };
